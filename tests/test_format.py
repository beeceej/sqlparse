...
class TestSpacesAroundOperators:
    ...
    def test_nested(self):
        ...
        assert self.formatter(sql) == "select *, case when a - b then c end from table"
...
class TestFormatReindent:
    ...
    def test_join(self):
        ...
        assert f(s) == "\n".join(["select *", "from foo", "inner join bar on 1 = 2"])
    ...
    def test_identifier_list_with_functions(self):
        f = lambda sql: sqlparse.format(sql, reindent=True)
        s = (
            "select 'abc' as foo, coalesce(col1, col2)||col3 as bar,"
            "col3 from my_table"
        )
        assert f(s) == "\n".join(
            [
    ...
    def test_long_identifier_list_with_functions(self):
        ...
        s = (
            "select 'abc' as foo, json_build_object('a',  a,"
    ...
    def test_insert_values(self):
        ...
        assert f(s) == "\n".join(["insert into foo", "values (1, 2)"])
...
class TestOutputFormat:
    def test_python(self):
        ...
        f = lambda sql: sqlparse.format(sql, output_format="python", reindent=True)
    ...
    def test_php(self):
        ...
        assert f(sql) == '$sql = "select * from foo;";'
        f = lambda sql: sqlparse.format(sql, output_format="php", reindent=True)
...
@pytest.mark.parametrize(
    "sql", ["select verrrylongcolumn from foo", 'select "verrrylongcolumn" from "foo"']
)
