...
@pytest.mark.parametrize(
    "s", ["x > DATE '2020-01-01'", "x > TIMESTAMP '2020-01-01 00:00:00'"]
)
...
@pytest.mark.parametrize(
    "s",
    ("select 1 where 1 = 2 union select 2", "select 1 where 1 = 2 union all select 2",),
)
    ...
    def validate_where_clause(where_clause, expected_tokens):
        ...
        for where_token, expected_token in zip(where_clause, expected_tokens):
            ...
            if where_token.ttype is not None:
                ...
                assert isinstance(where_token, expected_ttype) and re.match(
                    expected_value, where_token.value
                )
